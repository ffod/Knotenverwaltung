...
// NOTE: There are still some hardcoded freifunk-Stormarn things in the code, please adjust these to your needs!

$DB = "/var/www/vhosts/stormarn.freifunk.net/ffod-knoten";
$DEL_URL = "http://register.stormarn.freifunk.net/del.php";
$EDIT_URL = "http://register.stormarn.freifunk.net/edit2.php";
...
function get_node_by_key($key) {
  ...
  try {
    ...
    if (empty($result)) {
      print "Schl&uuml;ssel leider nicht gefunden.";
      die();
    }
    foreach ($result as $row) {
...
function keys_per_mail($tmp) {
    ...
    foreach ($tmp as $row) {
	    ...
	    $txt .= "VPN key (Router Schl&uuml;ssel): ".$row['key']."<br>\n";
	    $txt .= "Standort: ".$row['location']."<br>\n";
	    $txt .= "---------------------------------------------<br>\n";
    }

    $txt .= "Du kannst mit den VPN keys unter <a href='$edit_url'>diesem Link</a> editieren oder unter <a href='$del_url'>diesem Link</a> l&ouml;schen.<br>\nViele Gr&uuml;&szlig;e, Dein Freifunk-Stormarn Team.";
...
Hallo '.$name.'!<p>'.$txt;
  ...
  $header .= 'From: Freifunk Stormarn <noreply@stormarn.freifunk.net>'. "\r\n";
  $header .= 'Reply-to: noreply@stormarn.freifunk.net' ."\r\n";
...
function verify_mail($tmp) {
  ...
  $subject = 'Bitte best&auml;tigen';
  $link    = 'http://register.stormarn.freifunk.net/verify.php?email='.$to.'&hash='.$hash.'&node='.$knotenname;
  $message = '
Hallo '.$name.'!

Du bist in der Freifunk-Stormarn Datenbank als VerwalterIn des Knotens >'.$knotenname.'< vermerkt.<br>

Auf unserer Webseite wurde die L&ouml;schung Des Knotens >'.$knotenname.'< beantragt.<br>
Falls Du das warst, best&auml;tige die L&ouml;schung bitte, indem Du auf den unten stehenden Link klickst. Danke.<br>

<a href="'.$link.'">Hier zum Best&auml;tigen klicken</a><br>

Falls Du das nicht warst, wende Dich bitte an info@stormarn.freifunk.net<br>
...
function foot() {
  ...
  echo "<hr>";
  echo "<table><tr><td><small><a href='http://register.stormarn.freifunk.net/'>Neuer Knoten</a></small></td><td>|</td><td><small><a href='http://register.stormarn.freifunk.net/edit2.php'>Knoten bearbeiten</a></small></td><td>|</td><td><small><a href='http://register.stormarn.freifunk.net/del.php'>Knoten l&ouml;schen</a></small></td><td>|</td><td><small><a href='http://register.stormarn.freifunk.net/meine_knoten.php'>Meine Knoten anzeigen</a></small></td></tr></table>";
  
  echo "<small>Bei problemen mit dem Formular bitte eine mail an info(at)stormarn(Punkt)freifunk(Punkt)net</small><P>";
  echo "</body></html>";
